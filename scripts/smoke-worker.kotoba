...
;; (test/air_mro/route_test.cljc) はソースの判断を固定するが、bundle が
       ...
       (let [h (.-default m)]
         ...
         (-> (js/Promise.all
              #js [(call h "GET" "/") (call h "GET" "/health")
                   ...
                   (call h "POST" "/xrpc/com.etzhayyim.apps.airMro.createWorkOrder" "{}")
                   (call h "POST" "/xrpc/a/b" "{}")
                   (call h "GET" "/_app/meta")])
             (.then
              (fn [[page health bad pre nf mna single multi appmeta]]
                ...
                ;; 単一セグメントと同じ結末になることを比べて確かめる。中継先は
                ;; .invalid なので、この検査は実 DNS に依存しない —— 両方とも
                ;; 「上流に到達できなかった」502 になり、200 で隠れない。
                (check! "single-segment nsid is relayed (502, unreachable .invalid)" 502 (:status single))
                (check! "multi-segment nsid is relayed the same way, not rejected"
                ...
                ;; 移行前の src/app.ts にあった /_app/meta は持ち越していない
                (check! "/_app/meta was not carried over" 404 (:status appmeta))
                (let [f @failures]
                  (if (seq f)

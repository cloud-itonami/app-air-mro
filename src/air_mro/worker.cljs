(ns air-mro.worker
  ...
  ここには判断を置かない —— どのハンドラが答えるかは `air-mro.route/dispatch`
  が決め、ページの中身は `air-mro.view` が組む。どちらも `.cljc` なので、
  ブラウザもビルドも無しにテストできる。

  wrangler.jsonc の `main` は `dist/worker.js` を指し、それはこの名前空間を
  コンパイルしたものである。移行前は `svelte/.svelte-kit/cloudflare/_worker.js`
  —— tree に**存在しない**ビルド出力 —— を指していて、読み手が開く
  `src/app.ts` はどの bundle にも入っていなかった（docs/adr/0001）。

  `aget` を使うのは `:advanced-optimization` 下で env のキーが潰れないため
  （先例 `listingops.edge.worker` と同じ約束）。"
  (:require [air-mro.route :as route]
...
(defn fetch-handler [req env _ctx]
  (let [url (js/URL. (.-url req))
    ...
    (case action
      ...
      :health (json {:ok true :app "air-mro" :runtime "cljs"

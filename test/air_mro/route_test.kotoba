...
(deftest dispatch-page-and-health
  ...
  (testing "移行前の src/app.ts にあった /_app/meta は持ち越していない"
    (is (= :not-found (:action (route/dispatch "GET" "/_app/meta"))))))

(deftest dispatch-xrpc
  (testing "nsid をそのまま通す"
    (is (= {:action :xrpc :nsid "com.etzhayyim.apps.airMro.createWorkOrder"}
           (route/dispatch "POST" "/xrpc/com.etzhayyim.apps.airMro.createWorkOrder"))))
...
(deftest page-renders-what-it-is-handed-not-a-baked-table
  (testing "route 表を差し替えるとページも変わる。固定値なら落ちる"
    (let [fake [{:route/path "/only-this" :route/method :get :route/kind :page
                 :route/doc "差し替えた表"}]
          html (view/render {:css "" :routes fake :vars [] :mcp-url "https://x.invalid"})]
      (is (str/includes? html "/only-this"))
      (is (not (str/includes? html "/xrpc/:nsid"))
          "本物の route 表がページに焼かれている")))
  (testing "env のキーは渡されたものが出る"
    (let [html (view/render {:css "" :routes route/routes
                             :vars [:SOME_UNLIKELY_KEY_7F3A]
                             :mcp-url "https://x.invalid"})]
      (is (str/includes? html "SOME_UNLIKELY_KEY_7F3A")))))
